...
import { ForgotPasswordDto, RegisterDto, ResendVerificationDto, ResetPasswordDto, TwoFactorLoginDto } from './dto/auth.dto';
...
export class AuthService {
  ...
  async register(dto: RegisterDto, req: Request) {
    ...
    try {
      ...
      throw new BadRequestException('Registration failed: could not send verification email.');
    }
  ...
  // ─── Login ────────────────────────────────────────────────────────────────────
  async login(dto: TwoFactorLoginDto, req: Request) {
    ...
    if (user.isEmailOtpEnabled) {
      if (!dto.twoFactorCode) {
        ...
        await this.emailOtpService.sendLoginOtp(user.id);
        return {
          requires2FA: true,
          method: 'email_otp',
          message: 'A 6-digit OTP has been sent to your email. Enter it to complete login.',
        };
      }
      // Verify OTP code user entered
      await this.emailOtpService.verifyOtp(user.id, dto.twoFactorCode);
    }
    ...
    await this.activityLogService.log({
      ...
      metadata: {
        email: dto.email,
        twoFactorMethod: user.isEmailOtpEnabled ? 'email_otp' : 'none',
      },
      req,
    });
  ...
  async verifyEmail(token: string, req: Request) {
    ...
    await this.activityLogService.log({
      userId: user.id, action: ActivityAction.EMAIL_VERIFIED,
      metadata: { email: user.email }, req,
    });
  ...
  async resendVerification(dto: ResendVerificationDto, req: Request) {
    ...
    if (!user || user.isEmailVerified) {
      return { message: 'If this email exists and is unverified, a new email has been sent.' };
    }
    ...
    await this.activityLogService.log({
      userId: user.id, action: ActivityAction.RESEND_VERIFICATION,
      metadata: { email: user.email }, req,
    });
  ...
  async forgotPassword(dto: ForgotPasswordDto, req: Request) {
    ...
    await this.activityLogService.log({
      userId: user.id, action: ActivityAction.FORGOT_PASSWORD,
      metadata: { email: user.email }, req,
    });
  ...
  async resetPassword(token: string, dto: ResetPasswordDto, req: Request) {
    ...
    await this.activityLogService.log({
      userId: user.id, action: ActivityAction.PASSWORD_RESET,
      metadata: { email: user.email }, req,
    });
